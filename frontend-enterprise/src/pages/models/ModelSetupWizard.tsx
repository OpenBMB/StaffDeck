...
export default function ModelSetupWizard({
  ...
  // Returns the saved row, or null if validation failed before any request
  // was made (the caller has already set the error state in that case).
  async function persist(verify: boolean): Promise<ModelConfigRead | null> {
    if (!selectedChannel) return null;
    if (selectedChannel.category === 'custom' && customForm.extraBody.trim()) {
      try {
        JSON.parse(customForm.extraBody.trim());
      } catch {
        setSaveErrorMessage('额外参数必须是合法的 JSON 对象');
        setSaveResult('error');
        return null;
      }
    }
                  ...
                  <div className="grid grid-cols-2 gap-[14px]">
                    <LabeledField label="API 协议">
                      <Select
                        ...
                        onValueChange={(value) => setCustomForm((prev) => ({ ...prev, apiProtocol: value as ApiKeyProtocol }))}
                      >
