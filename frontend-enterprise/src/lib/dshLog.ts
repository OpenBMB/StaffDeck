import type { DshLogEntry } from '../api/dsh';
import { engineLabel, hookHandlerLabel, hookPointLabel, moduleLabel, operationLabel, toolLabel } from './dshLabels';
...
export function formatLogEntry(e: DshLogEntry): FormattedLog {
  const d = e.data ?? {};
  switch (e.type) {
    case 'session/start':
