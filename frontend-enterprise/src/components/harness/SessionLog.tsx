...
import type { HarnessLogEntry, HarnessSessionSummary } from '../../api/harness';
import { engineLabel } from '../../lib/harnessLabels';
import { formatLogEntry, logDate, logTime, type LogLevel } from '../../lib/harnessLog';
...
function sessionTitle(s: HarnessSessionSummary): string {
  const who = s.agent_name || '未指定员工';
  const what = s.title || s.session_id;
  return `${who} · ${what}`;
}

function LogLine({ e, tech }: { e: HarnessLogEntry; tech: boolean }) {
