...
import type { HarnessAssemblyState, HarnessInspectResult, HarnessTreeOption } from '../../api/harness';
import { KIND_META, operationLabel, placementSourceLabel, slotLabel } from '../../lib/harnessLabels';
...
export type ExternalModulesPanelProps = {
  assembly: HarnessAssemblyState | null;
  options: HarnessTreeOption[];
  tech: boolean;
  busy?: boolean;
  onInspect: (spec: string) => Promise<HarnessInspectResult | null>;
...
function InspectCard({ result, options, placements, tech, busy, onPlace }: { result: HarnessInspectResult; options: HarnessTreeOption[]; placements: Record<string, string>; tech: boolean; busy?: boolean; onPlace: (id: string, sub: string | null) => Promise<void> }) {
...
export default function ExternalModulesPanel({ assembly, options, tech, busy, onInspect, onAdd, onRemove, onPlace }: ExternalModulesPanelProps) {
  ...
  const [results, setResults] = useState<Record<string, HarnessInspectResult>>({});
