...
import type { DshAssembly, DshModule, DshTreeBig, DshTreeOption, DshTreeSub } from '../../api/dsh';
import { KIND_META, engineLabel, hookLabel, operationLabel, placementSourceLabel, slotLabel } from '../../lib/dshLabels';
...
const ENGINE_FOLLOWERS: Record<string, 'dsh' | 'legacy'> = { 'dsh.core': 'dsh' };
...
type Desired = { enabled: boolean; kind: 'switch' | 'choice' | 'fixed' | 'follows' | 'removed'; choiceLabel?: string; follows?: 'dsh' | 'legacy' };

/** 保存的装配里，这个模块应该是什么状态、能用什么方式改。 */
export function desiredState(m: DshModule, assembly: DshAssembly | null): Desired {
...
export function PlacementSelect({ value, options, onChange, disabled, compact }: { value: string | null; options: DshTreeOption[]; onChange: (subId: string | null) => void; disabled?: boolean; compact?: boolean }) {
  return (
...
function ModuleRow({ m, tech, assembly, options, busy, onToggleModule, onChooseEngine, onChooseProfile, onPlaceModule }: { m: DshModule } & Omit<ModuleTreeProps, 'tree' | 'loading'>) {
...
function desiredCount(modules: DshModule[], assembly: DshAssembly | null): number {
...
export default function ModuleTree({ tree, loading, ...rest }: ModuleTreeProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const hit = (m: DshModule) => [m.module_id, m.name, m.summary ?? '', m.slot, m.source ?? '', m.category ?? '', ...m.provides, ...m.provides.map(operationLabel), ...m.policy_actions].some((s) => s.toLowerCase().includes(needle));
    return tree
      .map((big) => ({
        ...big,
        subs: big.subs
