...
import type { DshBaseConnection, DshBaseConnectionUpdate, DshBaseTestResult } from '../../api/dsh';
...
function fromBase(b: DshBaseConnection | null | undefined): FormState {
  return {
...
function toUpdate(f: FormState): DshBaseConnectionUpdate {
...
export type BaseConnectionPanelProps = {
  base: DshBaseConnection | null;
  selected: boolean;           // 企业版 is the saved choice
  busy?: boolean;
  onSave: (patch: DshBaseConnectionUpdate) => Promise<boolean>;
  onTest: (patch?: DshBaseConnectionUpdate) => Promise<DshBaseTestResult | null>;
};
...
export default function BaseConnectionPanel({ base, selected, busy, onSave, onTest }: BaseConnectionPanelProps) {
  ...
  const [result, setResult] = useState<DshBaseTestResult | null>(null);
