# Harness v3 执行适配：保留 v2 的循环周期与状态语义
...
v2 与 Harness v3 共用该模块。这里的“使用 v2 的处理逻辑”指行为兼容，不再指调用旧 AgentLoop 方法。
...
当前 Harness v3 引擎 SDK 只暴露 `session/prompt`，没有公开的历史导入/恢复请求：

1. 同一温进程、同一逻辑循环且 checkpoint revision 一致时，复用原引擎会话。
2. 换进程、重启或原生历史领先于已提交 checkpoint 时，建立新的传输会话，并注入同一逻辑循环的公开 checkpoint。
3. 原生引擎会话 ID 可以在冷恢复时变化；业务循环 ID 和上下文归属不变。
4. 旧 Harness v3 标记式 checkpoint 缺少 transcript 时，按该循环自己的持久化 Run/Invocation 记录恢复可用公开历史；不混入其他 SOP 的记录，也不声称恢复从未持久化的内容。

## 结果提交不是业务能力

`CapabilityHost` 不再实现 `finish_task`。固定的 `StepCompletionPort` 属于 Bridge 的运行控制层：

- 普通对话：模型工具列表中不提供步骤提交接口，直接接收引擎最终输出并执行后处理。
...
- `test_sealed_e2e_engine.py`：真实 Harness v3 引擎 + 本机假模型，覆盖普通对话无结束工具、通用循环跨 TaskFrame、SOP 温/冷恢复、不同 SOP 隔离、完整调度器挂起/推进/完成、预算退出保留回执。
...
- 真实 Harness v3 引擎端到端组：6 passed；前端：249 passed；生产构建、i18n、配置检查通过。
