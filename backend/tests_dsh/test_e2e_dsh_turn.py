"""End-to-end: AgentLoop.handle_turn with dsh_enabled=True against the real model gateway.

Requires:
- DSH built at $DSH_ROOT (apps/cli/lib/bin.js)
...
Run:  set -a; source ../.codex-tmp/server-models.env; set +a
      DSH_ROOT=... .venv/bin/python -m pytest tests_dsh/test_e2e_dsh_turn.py -q -s
"""
...
DSH_ROOT = os.environ.get("DSH_ROOT") or "/Users/hm/Documents/UltraRAG4/.codex-tmp/dsh-inspect/deepseek-harness-dsh-v0.1.2-alpha.2"
...
def db(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DSH_ENABLED", "true")
...
def test_dsh_turn_knowledge_and_tool_with_pep_and_ledger(db: Session, tool_server: str) -> None:
    ...
    assert all((r.approval_json or {}).get("engine") == "dsh" for r in rows)
