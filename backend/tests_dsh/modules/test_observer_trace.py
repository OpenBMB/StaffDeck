...
from staffdeck_dsh.modules.builtin import TraceObserver
...
def test_provider_observer_trace(module) -> None:
    ...
    assert provider.on_event("t1", "s1", "dsh_turn_ended", {}) is None
...
def test_provider_observer_trace_receives_relayed_events(registry, monkeypatch) -> None:
    ...
    assert [et for et, _ in sink] == ["dsh_turn_started", "stream_delta", "harness_action_created"]
    assert [ev[2] for ev in seen] == ["dsh_turn_started", "stream_delta", "harness_action_created"]
