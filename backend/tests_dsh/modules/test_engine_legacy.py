"""Module test: ``engine.legacy`` (Harness v2 引擎, trusted, slot runtime.engine).

Provider ``LegacyEngine.open(loop, request, agent_id)`` returns the in-process
``HarnessV2Engine``. It is the deployment default whenever ``dsh_enabled`` is
...
from staffdeck_dsh.modules.builtin import LegacyEngine
...
def test_manifest_engine_legacy(registry, module):
    ...
    assert item.enabled is True, "dsh_enabled=False → Harness v2 is the active engine"
...
def test_manifest_engine_legacy_slot_is_guarded_even_without_fixture_marks(settings):
    reg = discover_and_install(ModuleRegistry(), settings)
    reg.seal()
...
def test_placement_engine_legacy(registry):
...
def test_placement_engine_legacy_shares_sub_with_dsh_engine(registry):
    _, sub, _ = _placed(registry)
    ids = sorted(x["module_id"] for x in sub["modules"])
    assert ids == ["engine.dsh", "engine.legacy"]
...
def test_disable_engine_legacy_not_switchable(registry):
    d = _described(registry)
    assert d["kind"] == "T"
...
def test_disable_engine_legacy_follows_dsh_enabled(settings):
    ...
    assert active is not None and active.manifest.module_id == "engine.dsh"
    assert MODULE_ID not in [i.manifest.module_id for i in reg.providers(SlotName.RUNTIME_ENGINE)]


def test_disable_engine_legacy_single_provider_slot_rejects_two_engines(settings):
    reg = discover_and_install(ModuleRegistry(), settings)
    reg.set_enabled("engine.dsh", True)  # both engines on → seal must refuse
...
def test_provider_engine_legacy_open_returns_harness_v2_engine(module, fake_loop, db):
...
def test_provider_engine_legacy_open_is_fresh_per_call(module, fake_loop):
...
def test_provider_engine_legacy_open_ignores_request_and_agent(module, fake_loop):
    """The legacy engine reads everything from the loop; request/agent_id are accepted for the contract only."""
    ...
    assert not hasattr(engine, "snapshot"), "legacy engine carries no DSH composition snapshot"


def test_provider_engine_legacy_is_what_engine_host_opens_by_default(module, fake_loop, settings):
    from app.core.harness_v2_engine import HarnessV2Engine
    from app.session.session_schema import ChatTurnRequest
    from staffdeck_dsh.bridge.engine_host import EngineHost

    host = EngineHost(settings)
    assert host.dsh_enabled is False
    request = ChatTurnRequest(tenant_id="t1", user_id="u1", agent_id="a1", message="hi")
...
def test_events_or_pep_engine_legacy_turn_contract_maps_and_denies_cross_tenant(registry, guard, security_ctx):
    d = _described(registry)
    assert d["policy_actions"] == []
    # engine.legacy provides runtime.turn/v1; the contract is mapped through DEFAULT_ACTION_MAP …
