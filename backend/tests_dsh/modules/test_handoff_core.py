...
from staffdeck_dsh.handoff.core import (
    TRANSITIONS,
    AssignmentStrategy,
...
from staffdeck_dsh.modules.kernel import HandoffCoreModule
...
def test_provider_handoff_core_build_falls_back_when_default_assignment_disabled(module, db, guard, monkeypatch):
    monkeypatch.setenv("STAFFDECK_DSH_DISABLED_MODULES", "handoff.assignment.default")
    reset_registry()
    core = module(MODULE_ID).provider.build(db, guard("handoff"))
