...
from staffdeck_dsh.contracts.security import DEFAULT_ACTION_MAP, PolicyActionMapper
from staffdeck_dsh.events import relay as relay_mod
from staffdeck_dsh.events.relay import SessionEventRelay, fanout_event
from staffdeck_dsh.modules import registry as registry_mod
from staffdeck_dsh.modules.builtin import FeedbackObserver
...
def test_disable_observer_feedback_via_env_and_comma_list(settings, monkeypatch) -> None:
    settings.dsh_disabled_modules = " observer.feedback , unknown.module "
    reg = discover_and_install(ModuleRegistry(), settings)
    reg.seal()
...
def test_provider_observer_feedback(module) -> None:
    ...
    assert provider.on_event("t1", "s1", "dsh_assistant_message", {"interrupted": False, "execution_engine": "dsh"}) is None
    assert provider.on_event("t1", "s1", "dsh_turn_ended", {"turn": 1, "reason": "stop"}) is None
...
def test_provider_observer_feedback_receives_relayed_events(registry, monkeypatch) -> None:
    ...
    assert [ev[2] for ev in seen] == ["dsh_assistant_message", "dsh_turn_ended", "harness_tool_result"]
    assert seen[0][3] == {"interrupted": True, "execution_engine": "dsh"}
    assert seen[1][3] == {"turn": 2, "reason": "stop"}
    assert seen[2][3] == {"call_id": "c1", "is_error": True, "execution_engine": "dsh"}
...
def test_provider_observer_feedback_can_be_replaced_by_explicit_registration(registry, monkeypatch) -> None:
    ...
    fanout_event("t1", "s1", "dsh_turn_ended", {"turn": 1})
    assert custom.calls == ["dsh_turn_ended"]
    assert builtin_seen == ["dsh_turn_ended"]  # registry observers are distinct objects, both consume
...
def test_events_observer_feedback_failure_never_breaks_the_turn(registry, monkeypatch) -> None:
    ...
    fanout_event("t1", "s1", "dsh_turn_ended", {"turn": 1})  # must not raise
    assert other_seen == ["dsh_turn_ended"]  # siblings still receive the event
