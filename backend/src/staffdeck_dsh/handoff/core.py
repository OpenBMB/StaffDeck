...
class HandoffCore:
    ...
    def create(self, ctx: SecurityContext, session: ChatSession, *, pending_question: str, context_summary: str, trigger_skill_id: str | None = None, trigger_step_id: str | None = None, metadata: Mapping[str, Any] | None = None, notify_channel: str | None = None) -> HumanHandoffRequest:
        ...
        self.db.add(session)
        self.db.add(AgentEvent(tenant_id=row.tenant_id, session_id=row.session_id, event_type="human_handoff_created", payload_json={"handoff_id": row.id, "trigger_skill_id": trigger_skill_id, "trigger_step_id": trigger_step_id, "engine": "dsh"}))
        self.db.commit()
