...
_ENV_FIELDS = ("dsh_enabled", "security_profile", "dsh_disabled_modules", "dsh_modules", *BaseConnection._SETTINGS_FIELDS.values())
...
def defaults_from_settings(settings: Any) -> RuntimeOverrides:
    ...
    return RuntimeOverrides(
        engine="dsh" if bool(env_value(settings, "dsh_enabled")) else "legacy",
        security_profile=str(env_value(settings, "security_profile") or "OSS_LOCAL"),
        disabled_modules=_split(env_value(settings, "dsh_disabled_modules")),
        extra_modules=_split(env_value(settings, "dsh_modules")),
    ).normalized()
