...
from staffdeck_dsh.contracts.invocation import ModuleInvocation, ModuleResult, Receipt
...
class InvocationLedger:
    ...
    def start(self, invocation: ModuleInvocation) -> LedgerEntry:
        ...
        row = HarnessInvocationRecord(
            ...
            approval_json={"engine": "dsh", "snapshot_id": ctx.snapshot_id, "binding_id": invocation.binding_id},
        )
        self.db.add(row)
        try:
            self.db.commit()
