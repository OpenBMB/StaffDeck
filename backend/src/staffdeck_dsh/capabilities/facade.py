...
from staffdeck_dsh.contracts.invocation import InvocationContext, ModuleInvocation, ModuleResult
...
class SandboxFacade:
    ...
    def discover_artifacts(self, task_frame_id: str) -> list[dict[str, Any]]:
        ...
        for raw in discovered:
            ...
            name = Path(rel).name
            item.update({"sandbox_path": _sandbox_path(rel), "display_name": name, "content_type": mimetypes.guess_type(name)[0] or "application/octet-stream", "source": "dsh.workspace_discovery"})
            out.append(item)
        return out
