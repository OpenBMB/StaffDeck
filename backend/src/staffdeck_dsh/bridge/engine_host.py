...
``DshEngine`` *is* a ``HarnessV2Engine`` — it inherits the whole outer turn
...
invoker.invoke, ...)`` and that call now lands on ``DshTaskAgent``.

The per-frame turn context the DSH agent needs (snapshot, security context,
...
``EngineHost.open(loop, request)`` returns the engine for this turn:
    ...
    settings.dsh_enabled is False        → HarnessV2Engine (legacy, default)
    settings.dsh_enabled is True         → DshEngine, unless the Staff is
                                            excluded by ``dsh_staff_allowlist``
    settings.dsh_staff_allowlist set     → only listed agent ids run on DSH
...
from staffdeck_dsh.bridge.worker import DshWorkerConfig
from staffdeck_dsh.composition.compiler import CompositionCompiler, CompositionSnapshot
...
def get_runtime(settings: Any) -> DshRuntime:
    global _runtime
    with _runtime_lock:
        if _runtime is None:
            root = Path(str(_settings_value(settings, "dsh_root", "") or "")).expanduser()
            if not str(root):
                raise EngineUnavailable("dsh_root is not configured")
            home = Path(str(_settings_value(settings, "dsh_home", "") or "") or (root / ".staffdeck-dsh-home")).expanduser()
            cfg = DshWorkerConfig(
                dsh_root=root,
                dsh_home=home,
                node_bin=str(_settings_value(settings, "dsh_node_bin", "node") or "node"),
                permission_mode=str(_settings_value(settings, "dsh_permission_mode", "danger-full-access") or "danger-full-access"),
                initialize_timeout_seconds=float(_settings_value(settings, "dsh_initialize_timeout_seconds", 90.0) or 90.0),
                request_timeout_seconds=float(_settings_value(settings, "dsh_request_timeout_seconds", 600.0) or 600.0),
            )
            cfg.validate()
            rt = DshRuntime(worker_config=cfg)
            rt.start()
            _runtime = rt
        return _runtime
...
class DshEngine(HarnessV2Engine):
    ...
    def _ensure_turn_context(self, request: ChatTurnRequest, session: ChatSession) -> None:
        ...
        self.snapshot = self.composition_compiler.compile(staff, generation=0, metadata={"turn_id": self.user_message_id, "engine": "dsh"})
    ...
    def _run_frame(  # type: ignore[override]
        ...
        self.task_agent = DshTaskAgent(self.runtime, turn)  # type: ignore[assignment]
...
class EngineHost:
    ...
    def dsh_enabled(self) -> bool:
        value = _settings_value(self.settings, "dsh_enabled", False)
