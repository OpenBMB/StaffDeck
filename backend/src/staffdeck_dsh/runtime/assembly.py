...
offer a restart. ``restart_dsh_runtime`` first *preflights* the saved assembly
...
from staffdeck_dsh.modules.config import (
    InvalidConnection,
    RuntimeOverrides,
    apply_overrides,
...
from staffdeck_dsh.modules.registry import ModuleRegistry, build_registry, install_registry, reset_registry
from staffdeck_dsh.security.profile import build_profile, install_profile, reset_profile
...
def _activate(settings: Any, overrides: RuntimeOverrides, reg: ModuleRegistry, profile: Any) -> dict[str, Any]:
    """Make the prepared components live and (if enabled) boot the DSH runtime."""
    ...
    if bool(getattr(settings, "dsh_enabled", False)):
        try:
            ...
            if bool(getattr(settings, "dsh_fallback_to_legacy", True)):
                logger.warning("DSH runtime unavailable (%s); turns fall back to legacy", exc)
...
def restart_dsh_runtime(settings: Any, *, tenant_id: str | None = None, principal_id: str | None = None) -> dict[str, Any]:
    """Rebuild the runtime from the saved assembly.

    Order: preflight (no global state touched) → build new registry/profile in
    locals → stop the DSH process → swap registry/profile/settings atomically →
    boot DSH. Hosts see either the old or the new assembly, never a default one.
    In-flight turns lose their DSH process; that is the documented cost. If the
    ...
    try:
        ...
        from staffdeck_dsh.security.profile import peek_profile
