...
class SopSessionState:
    ...
    def enqueue_task_frame(self, session: ChatSession, frame: dict[str, Any]) -> None:
        ...
        if task_id and any(
            isinstance(item, dict) and str(item.get("task_id") or "") == task_id for item in frames
        ):
            return
    ...
    def _remove_task_frame(self, session: ChatSession, task_id: str | None) -> None:
        if not task_id:
            return
        session.pending_tasks_json = _without_task_or_skill(
            session.pending_tasks_json, task_id=task_id
        )
...
def _set_active_task_id(session: ChatSession, task_id: str | None) -> None:
    if not task_id:
        if (
            isinstance(session.awaiting_input_json, dict)
            and "task_id" in session.awaiting_input_json
        ):
...
def _pop_last_skill_frame(
    ...
    for index in range(len(stack) - 1, -1, -1):
        if (
            stack[index].get("skill_id") == skill_id
            or stack[index].get("target_skill_id") == skill_id
        ):
...
def _without_task_or_skill(
    ...
    for frame in list(frames_json or []):
        if task_id and str(frame.get("task_id") or "") == task_id:
            continue
        if skill_id and (
            frame.get("skill_id") == skill_id or frame.get("target_skill_id") == skill_id
        ):
            continue
        frames.append(frame)
    return frames
    ...
    merged.update(
        {
            ...
            if key not in {"task_id", "created_at", "slots", "slot_hints"}
...
def _patch_task_frame(
    frames_json: list[dict] | None, task_id: str, patch: dict[str, Any]
) -> list[dict]:
    frames = []
    for frame in list(frames_json or []):
        if isinstance(frame, dict) and str(frame.get("task_id") or "") == task_id:
...
def _frame_with_slot_hints(
    frame: dict[str, Any] | None, slot_hints: dict | None
) -> dict[str, Any] | None:
    ...
    merged_slots = strip_router_generated_message_slots(
        {**current_hints, **current_slots, **incoming_hints}
    )
