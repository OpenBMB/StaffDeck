"""Tool-aware model protocol adapters shared by StaffDeck model clients.
...
def _completion(
    cid: str, model: str, message: dict[str, Any], finish_reason: str, usage: dict[str, Any] | None
) -> dict[str, Any]:
    out: dict[str, Any] = {
        "id": cid,
...
def _usage(prompt: Any, completion: Any, total: Any = None) -> dict[str, Any] | None:
    ...
    out = {
        k: v
        for k, v in (("prompt_tokens", p), ("completion_tokens", c), ("total_tokens", t))
        if v is not None
    }
    return out or None
...
def anthropic_payload(wire: dict[str, Any]) -> dict[str, Any]:
        ...
        if role == "assistant":
            ...
            for call in message.get("tool_calls") or []:
                ...
                blocks.append(
                    {
                        "type": "tool_use",
                        "id": str(call.get("id") or _new_call_id()),
    ...
    if thinking:
        budget = min(
            max(_ANTHROPIC_MIN_THINKING_BUDGET, max_tokens // 2), _ANTHROPIC_MAX_THINKING_BUDGET
        )
...
def _anthropic_completion(response: Any, model: str) -> dict[str, Any]:
    ...
    for block in _get(response, "content") or []:
        ...
        if kind == "text":
            ...
            tool_calls.append(
                {
                    "id": str(_get(block, "id") or _new_call_id()),
                    "type": "function",
                    "function": {
    ...
    return _completion(
        str(_get(response, "id") or _new_id()),
        model,
    ...
    try:
        for event in events:
            ...
            if kind == "message_start":
                ...
                if delta_type == "text_delta":
                    ...
                    yield _chunk(
                        cid, model, {"reasoning_content": str(_get(delta, "thinking") or "")}
                    )
                ...
                if index is not None and index not in args_seen:
                    ...
                    yield _chunk(
                        cid,
                        model,
                        {"tool_calls": [{"index": index, "function": {"arguments": "{}"}}]},
                    )
        ...
        if not finished:
            yield _chunk(
                ...
                model,
                {},
                finish_reason="tool_calls" if next_tool else "stop",
                usage=_usage(prompt_tokens, completion_tokens),
            )
    finally:
        _close(events)
...
def responses_payload(wire: dict[str, Any]) -> dict[str, Any]:
    ...
    if functions:
        ...
        if choice:
            mode, name = choice
            payload["tool_choice"] = (
                {"type": "function", "name": name} if mode == "function" else mode
            )
    return payload
...
def _responses_usage(usage: Any) -> dict[str, Any] | None:
    return _usage(
        _get(usage, "input_tokens"), _get(usage, "output_tokens"), _get(usage, "total_tokens")
    )
...
def _responses_completion(response: Any, model: str) -> dict[str, Any]:
    ...
    return _completion(
        ...
        model,
        _assistant_message("".join(text), tool_calls, "".join(reasoning)),
        _responses_finish(response, bool(tool_calls)),
        _responses_usage(_get(response, "usage")),
    )
...
def _responses_events(events: Any, model: str) -> Iterator[dict[str, Any]]:
    ...
    try:
        for event in events:
            ...
            if kind == "response.created":
                            ...
                            "tool_calls": [
                                {
                                    "index": index,
                                    "id": str(
                                        _get(item, "call_id") or _get(item, "id") or _new_call_id()
                                    ),
                                    "type": "function",
                                    "function": {
                                        "name": str(_get(item, "name") or ""),
                ...
                yield _chunk(
                    ...
                    model,
                    {},
                    finish_reason=_responses_finish(response, next_tool > 0),
                    usage=usage,
                )
        ...
        if not finished:
            yield _chunk(
                cid, model, {}, finish_reason="tool_calls" if next_tool else "stop", usage=usage
            )
    finally:
        _close(events)
...
class GeminiAdapter:
    ...
    def stream(self, wire: dict[str, Any]) -> Iterator[dict[str, Any]]:
        ...
        ctx = self._http.stream(
            "POST",
            _gemini_endpoint(self._base_url, model, "streamGenerateContent", stream=True),
            headers=_gemini_headers(self._api_key),
            json=gemini_payload(wire),
        )
