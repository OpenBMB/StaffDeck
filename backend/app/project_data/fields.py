...
class FieldDefinition:
    ...
    def __init__(
        ...
        label: str,
        value_type: str = "text",
        information_domain: str = "project",
        scope: str = "project",
        required: bool = False,
        editable: bool = True,
        ...
        self.information_domain = information_domain
        self.scope = scope
        self.required = required
        self.editable = editable
        self.sync_policy = sync_policy
        self.validator_name = validator_name
        self.source_optional = source_optional
...
SYSTEM_FIELD_DEFINITIONS = [
    FieldDefinition(
        field_key=key,
        label=_TEXT_FIELDS.get(key, key.rsplit(".", 1)[-1]),
        value_type=_VALUE_TYPES.get(key, "text"),
        information_domain=key.split(".", 1)[0],
        scope="project",
        required=key in {"organization.legal_name", "certification_project.scope"},
        validator_name=_VALUE_TYPES.get(key),
    )
...
def validate_candidate_request(request: ProjectDataCandidateCreate) -> None:
    if _is_empty(request.value):
        raise ProjectFieldValidationError("EMPTY_VALUE")
    if request.source is None:
        raise ProjectFieldValidationError("SOURCE_REQUIRED")
    if not request.source.location.strip() or not request.source.evidence_excerpt.strip():
        raise ProjectFieldValidationError("SOURCE_REQUIRED")
