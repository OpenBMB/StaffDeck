...
class TurnCoordinator:
    """Engine-neutral outer orchestration. Model phases are injected collaborators.

    The AgentLoop compatibility owner supplies existing domain services, but model/memory
    collaborators are owned by this coordinator and never monkey-patched onto that owner.
    """

    def __init__(self, owner: Any, *, services: Any = None) -> None:
        from app.core.turn_services import TurnServices

        self.services = services or TurnServices.from_agent_loop(owner)
        self.owner = owner
        self.db = owner.db
        self.events = owner.events
        self.memory = getattr(owner, "memory", None)
        self.capture_memory = getattr(owner, "_enqueue_memory_capture", None)
        self._legacy_capture = self.capture_memory
        self.response_generator = getattr(owner, "response_generator", None)
    ...
    def _prepare_modules(self, request: ChatTurnRequest, session: ChatSession) -> None:
        if self.registry is None:
            return
        from staffdeck_harness.memory import (
            ProviderMemoryFacade,
        ...
        if callable(self.capture_memory):
            self.capture_memory = facade.bind_capture(self.events, self._legacy_capture)

    def _team_provider(self):
        from staffdeck_harness.contracts.manifest import SlotName
        from staffdeck_harness.modules.kernel import TeamProviderModule

        if self.registry is None:
            return TeamProviderModule()
        candidates = self.registry.selected(SlotName.STAFF_TEAM, getattr(self, "snapshot", None))
        if len(candidates) != 1:
            raise RuntimeError("团队协作模块未启用或存在多个未明确绑定的实现")
        from staffdeck_harness.runtime.team_host import TeamHost

        return TeamHost(candidates[0].provider)
    ...
    def supervision_required(self, skill) -> bool:
        """Must the final reply be buffered until output supervision has seen it?

        Only when a turn_stopping hook exists that can actually refuse or rewrite it: any hook
        while an SOP is active, or a non-built-in hook in ordinary conversation. Otherwise the
        reply streams to the client as it is generated, exactly as before the module runtime
        existed. ``supervise_reply`` still runs afterwards (it is a passthrough in that case).
        """

        if self.registry is None or getattr(self, "snapshot", None) is None:
            return False
        from staffdeck_harness.composition.compiler import compile_hooks

        sop_id = skill.skill_id if skill else None
        plan = compile_hooks(self.registry.hooks(self.snapshot, sop_id=sop_id))
        handlers = [c.handler for c in plan.order.get("turn_stopping", ())]
        if not handlers:
            return False
        if sop_id:
            return True
        return any(h not in self._SOP_ONLY_SUPERVISORS for h in handlers)

    def supervise_reply(self, request, session, reply: str, skill, step_result) -> str:
        if self.registry is None or getattr(self, "snapshot", None) is None:
            return reply
        from staffdeck_harness.composition.compiler import compile_hooks
        from staffdeck_harness.contracts.hooks import HookContext
        from staffdeck_harness.contracts.errors import ModuleSdkError
        from staffdeck_harness.interactions.pipeline_host import (
            InteractionPipelineHost,
            PipelineState,
        )

        sop_id = skill.skill_id if skill else None
        pipeline = InteractionPipelineHost(
            compile_hooks(self.registry.hooks(self.snapshot, sop_id=sop_id)),
            self.registry.hook_handlers(self.snapshot, sop_id=sop_id),
        )
        state = PipelineState(
            self.snapshot, session_slots=dict(session.slots_json or {}), active_sop_id=sop_id
        )
        ctx = HookContext(
            "turn_stopping",
            request.tenant_id,
            self.snapshot.staff_id,
            session.id,
            self.user_message_id or session.id,
            1,
            self.snapshot.snapshot_id,
            {"final_text": reply, "phase": "response"},
            generation=self.snapshot.generation,
        )
        decision = pipeline.run("turn_stopping", ctx, state)
        if decision.kind in {"deny", "steer"}:
            raise ModuleSdkError(
                decision.reason or "final output was refused by supervision", code="OUTPUT_DENIED"
            )
        if decision.replacement is not None:
            replacement = decision.replacement
            if not replacement.success or not isinstance(replacement.data, str):
                raise ModuleSdkError("invalid final output projection", code="OUTPUT_DENIED")
            reply = replacement.data
        if decision.handoff:
            self.services.create_human_handoff_request(
                request.tenant_id,
                session,
                skill,
                step_result.model_copy(
                    update={"handoff": True, "action": "handoff", "reply": reply}
                ),
            )
            return "已为你提交人工处理请求，请稍候，工作人员会尽快回复。"
        return reply
    ...
    def _run(self, request: ChatTurnRequest) -> ChatTurnResponse:
        ...
        if session_request.session_id:
            ...
            self._session_lock = acquire_harness_session(session_request.session_id)
        session = self._get_or_create_session(session_request)
        self.session = session
        self._prepare_modules(request, session)
        ...
        user_message = self.services.append_message(
            ...
            "user",
            request.message,
            metadata=self.services.user_message_metadata(request),
        )
        ...
        execution_message = (
            slash_command_message(self.slash_command) if self.slash_command else request.message
        )
        ...
        conversation_context = self.services.conversation_context(
        ...
        if request.interaction_mode == "team_tl" and session.team_id:
            ...
            if team is not None and team_context is None:
                team_context = self._team_provider().planner_context(self.db, team)
        ...
        if remote_frames:
            ...
            team_publish_result = self._team_provider().publish(
                self.db,
                team=team,
                session=session,
        ...
        remaining_turn_actions = max(
            0,
            int(self.services.get_agent_loop_max_actions(request.tenant_id, session.agent_id)),
        )
        ...
        for record_index, row in enumerate(records):
            ...
            if row.status == "completed":
                ...
                if released:
                    ...
                    known_record_ids.update(item.task_id for item in released)
                    self.events.record(
                        request.tenant_id,
                        session.id,
        ...
        if team_publish_result is not None and not execution_results:
            ...
            reply = f"已完成 {task_count} 个团队任务的拆分与派发。"
        ...
        if is_handoff:
            ...
            response_args = (
                ...
                None,
                model_config,
                self.services.get_persona_prompt(request.tenant_id, session.agent_id),
                memory_context,
        ...
        self._raise_if_cancelled(request, session)
        if self.services.stream_sink is not None:
            self.services.stream_finished(self.services.stream_sink.finish())
        self.turn_store.begin_completion(self.turn_record)
        reply = self.services.finalize_turn(
            session,
            request.tenant_id,
            reply,
            last_step_result,
    ...
    def _renew_execution_leases(
        ...
        if not lease_owner or attempt_no is None:
            raise HarnessExecutionFenced("Harness TaskFrame lease token is missing.")
        try:
            self.session_leases.renew(self.session_lease)
            self.turn_store.renew(self.turn_record)
    ...
    def _run_frame(
        ...
        step_timeout_seconds = (
            _skill_step_timeout_seconds(active_skill) if frame.kind == "sop" else None
        )
        self.events.record(
            request.tenant_id,
            session.id,
        ...
        while remaining_actions > 0:
            ...
            if frame.kind == "conversation":
                if frame.decision == "handoff_human" or result.status == "handoff":
                    result.status = "handoff"
                    last_step_result = _step_result(result)
                    handoff = self.services.create_human_handoff_request(
                        request.tenant_id,
                        session,
                        None,
                        last_step_result,
    ...
    def _restore_visible_active_frame(
        ...
        if session.status == "handoff":
            ...
            return
        awaiting_conversation = self.store.latest_awaiting_conversation(session)
        if awaiting_conversation is not None:
            ...
            session.awaiting_input_json = {
                ...
                "kind": "conversation",
                "expected_fields": list(requirement.get("required_slots") or []),
                "requirements": list(awaiting_conversation.requirements_json or []),
        ...
        original_records = [row for row in records if row.skill_id == original_skill_id]
        ...
        candidates = [
            row
            for row in records
            if row.kind == "sop" and row.status not in {"completed", "cancelled", "failed"}
        ]
        if original_records:
            candidates.sort(
                key=lambda row: (
                ...
                "awaiting_input": (
                    {
                        ...
                        "skill_id": row.skill_id,
                        "step_id": row.step_id,
                        "expected_fields": (row.task_requirement_json.get("required_slots") or []),
                        "question_summary": reply or None,
                    }
...
def _defer_failed_step_after_completed_checkpoint(
    ...
    summaries = [
        summary for summary in (checkpoint.task_summary.strip(), failure_summary) if summary
    ]
...
def _combine_results(
    ...
    last = results[-1]
    terminal_reply = next(
        (item.reply_fragment.strip() for item in reversed(results) if item.reply_fragment.strip()),
        "",
    )
    return TaskExecutionResult(
        task_frame_id=task_frame_id,
        ...
        slot_updates={key: value for item in results for key, value in item.slot_updates.items()},
        next_step_id=last.next_step_id,
        citations=[citation for item in results for citation in item.citations],
        evidence_results=[evidence for item in results for evidence in item.evidence_results],
        capability_results=[
            capability_result for item in results for capability_result in item.capability_results
        ],
        structured_result=last.structured_result,
        artifacts=[artifact for item in results for artifact in item.artifacts],
        task_summary="；".join(
            dict.fromkeys(
                item.task_summary.strip() for item in results if item.task_summary.strip()
            )
        ),
...
def _structured_reply_requires_synthesis(reply: str, structured_result: Any) -> bool:
    ...
    looks_like_json = reply.startswith("{") or (
        reply.startswith("[") and (len(reply) == 1 or reply[1:2] in {"{", "[", '"'})
    )
...
def _response_task_payload(
    ...
    return {
        ...
        "tool_result": (
            {
                "success": all(bool(item.get("success")) for item in result.capability_results),
...
def _inject_handoff_context(
    ...
    is_resume_turn = (
        bool(request and getattr(request, "channel", "") == "human_handoff_resume") and is_answered
    )
...
def get_or_create_harness_session(
    ...
    if session.tenant_id != request.tenant_id:
        raise HarnessExecutionFenced("Harness session tenant does not match the request.")
    if request.user_id is not None and session.user_id != request.user_id:
        raise HarnessExecutionFenced("Harness session user does not match the request.")
    if request.agent_id is not None and session.agent_id not in {None, request.agent_id}:
        raise HarnessExecutionFenced("Harness session agent does not match the request.")
...
def _restore_session_state(session: ChatSession, state: dict[str, Any]) -> None:
...
def _dependency_order(
    ...
    while remaining:
        ready = [
            ...
            if all(
                dependency_id in resolved or dependency_id not in by_id
