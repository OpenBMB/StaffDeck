...
from staffdeck_harness.runtime import services as runtime_services
...
def test_worker_recovers_from_exception_using_fresh_session(monkeypatch):
    ...
    monkeypatch.setattr(worker, "_dispatcher", None)
    monkeypatch.setattr(
        runtime_services,
        "maintenance_sessions",
        lambda: iter([sessions.append(object()) or sessions[-1]]),
    )
    monkeypatch.setattr(
        worker,
        "ExternalTaskDispatcher",
        lambda: SimpleNamespace(dispatch=lambda *_args: False, close=lambda: None),
    )
    monkeypatch.setattr(
...
def test_worker_shutdown_does_not_spawn_overlapping_generation(monkeypatch):
    ...
    monkeypatch.setattr(runtime_services, "maintenance_sessions", lambda: iter([None]))
    monkeypatch.setattr(
        worker,
        "ExternalTaskDispatcher",
        lambda: SimpleNamespace(dispatch=lambda *_args: False, close=lambda: None),
