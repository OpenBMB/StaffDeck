"""The Harness v3 subprocess never talks to a provider: every model call goes through the bridge's
...
def test_streaming_passes_provider_chunks_and_applies_model_config():
    ...
    finished = traces[-1][1]
    assert finished["engine"] == "harness_v3" and finished["input_tokens"] == 10 and finished["output_tokens"] == 5 and finished["finish_reason"] == "tool_calls"
