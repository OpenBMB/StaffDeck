...
def test_disabled_module_list_toggles_before_seal() -> None:
    s = _Settings()
    s.harness_disabled_modules = "handoff.notifier.dingtalk,handoff.notifier.wecom"
    reg = discover_and_install(ModuleRegistry(), s)
...
def test_module_requiring_unprovided_operation_fails_seal() -> None:
    reg = ModuleRegistry()
    reg.install(__import__("staffdeck_harness.modules.registry", fromlist=["manifest"]).manifest("test.needy", "Needy", kind="A", slots=[SlotName.STAFF_INTERACTION], requires=["knowledge.search/v1"]), object(), slot=SlotName.STAFF_INTERACTION)
...
def test_taxonomy_tree_covers_every_registered_module_once() -> None:
    from staffdeck_harness.modules.taxonomy import TAXONOMY, tree
...
def test_install_validates_module_identity() -> None:
    ...
    from staffdeck_harness.modules.registry import manifest as mk
...
def test_env_snapshot_lets_admin_values_be_cleared_back_to_env(tmp_path) -> None:
    from staffdeck_harness.modules import config as cfg
    ...
    class S:
        ...
        harness_disabled_modules = ""
        harness_modules = ""
        harness_v3_home = str(tmp_path)
        harness_runtime_config_path = ""
...
def test_preflight_rejects_policy_actions_on_unguarded_slot(tmp_path, monkeypatch) -> None:
    ...
    class S:
        ...
        harness_modules = ""
        harness_v3_home = str(tmp_path)
        harness_runtime_config_path = str(tmp_path / "rt.json")
        base_authz_url = ""
