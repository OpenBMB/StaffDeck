...
from app.core import turn_coordinator as turn_coordinator_module
...
def main(expected_mode: str | None = None) -> int:
    ...
    with Session(database) as db:
        ...
        overall_agent = AgentProfile(
            id="agent-parity-overall",
            tenant_id="tenant-parity",
            name="Parity Overall",
            is_overall=True,
            harness_max_actions=int(limits.get("remainingActions") or 6),
            metadata_json={"owner_user_id": "user-parity"},
        )
        ...
        db.add(
            User(
                ...
                role="admin",
                password_hash="parity-fixture-not-for-authentication",
            )
        )
        db.add(model)
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch.object(SnapshotManifestBuilder, "build", deterministic_manifest)
            )
            stack.enter_context(
            ...
            stack.enter_context(
                patch.object(
                    turn_coordinator_module._HarnessV3SidecarCapabilityInvoker,
                    "invoke",
                    lambda _self, name, arguments: invoke_external_tool(
                        _self,
                        f"parity-{name}",
                        {},
                        name,
                        arguments,
                        call_id=f"parity-{name}",
                    ),
                )
            )
