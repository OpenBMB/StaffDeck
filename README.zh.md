...
<details>
<summary><strong>之前部署的版本，怎么升级到最新 release？</strong></summary>

客户端安装版：下载最新安装包覆盖安装即可，数据目录不受影响，首次启动时会自动完成数据库结构升级。源码部署：停止应用、备份数据库和 `backend/.env`、拉取最新代码、重装依赖后重启。详细步骤见[升级指南](docs/upgrade-guide.md)（英文）。
</details>
