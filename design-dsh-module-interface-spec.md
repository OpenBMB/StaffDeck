...
| 8 | 引擎驱动（StaffDeck → DSH） | `bridge/worker.py · DshProcess` → `dsh --profile sdk` 子进程 | JSON-RPC 2.0 NDJSON over stdio（官方 Python SDK `HarnessClient`） | 进程环境变量；子进程**不持有任何模型提供方凭证** | 无；Turn 取消 = 关闭子进程 | 【已实现】（内部接口） |
