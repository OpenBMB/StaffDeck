...
> 适用范围：`backend/src/staffdeck_dsh`（Harness v3 引擎桥接 + 模块注册表 + 管理后台 `/admin`）。本文是**约束性契约**：任何模块（内置或第三方）要接入 StaffDeck 运行时，必须遵守这里写死的接入方式、报文形状与约定。
>
> - 每条条款标注 **【已实现】** 或 **【待实现】**。【已实现】条款以当前代码为准，出处写成 `文件路径 · 符号名`（函数/类/常量），不写行号；【待实现】条款给出精确设计，供后续按此实现。
> - 关键词 MUST / MUST NOT / SHOULD / MAY 按 RFC 2119 理解。
> - 术语沿用管理后台文案：Harness v3 引擎 = `engine.dsh`（DSH bridge），Harness v2 引擎 = `engine.legacy`（内置）；权限模式：开源版 · 本地权限 = `OSS_LOCAL`，企业版 · 统一权限中心 = `BUSINESS_BASE`。
...
|---|---|---|---|---|---|---|
| 1 | 模块注册 `register(registry, ctx)` | 装配（`modules/registry.py · discover_and_install`）→ 模块包 | 进程内 Python：entry point 组 `staffdeck_dsh.modules` 或规格串 `pkg.mod:register` | 进程信任；只有租户管理员能通过 `/admin` 添加规格串 | 每次 restart 重跑，MUST 幂等；失败即整次装配失败 | 【已实现】 |
...
| 8 | 引擎驱动（StaffDeck → DSH） | `bridge/worker.py · DshProcess` → `dsh --profile sdk` 子进程 | JSON-RPC 2.0 NDJSON over stdio（官方 Python SDK `HarnessClient`） | 进程环境变量；密钥用 `!!js process.env.*` 引用 | 无；Turn 取消 = 关闭子进程 | 【已实现】（内部接口） |
...
| 11 | Webhook 入站（第三方 → StaffDeck） | 第三方 → `POST /api/enterprise/dsh/hooks/{module_id}/inbound` | HTTPS POST JSON | 同一 HMAC 方案或 Bearer workload token | `delivery_id` 去重；重复返回首次应答 | 【待实现】 |
...
| 15 | 管理 API | 前端 `/admin` → `api/admin.py · router`（`/api/enterprise/dsh/*`） | HTTP JSON | 登录 JWT + `ensure_tenant_admin`（读接口 `ensure_current_user_tenant`；`/status` 对非管理员隐去路径 / 错误详情） | `/restart` 先预检、再在局部构建、最后原子切换；每次保存 / 测试 / 预检 / 重启写审计事件（`GET /audit`） | 【已实现】 |
...
`contracts/manifest.py · ModuleKind`，前端标签见 `frontend-enterprise/src/lib/dshLabels.ts`。
...
【已实现】`switchable = kind is CODE or bool(metadata.switchable)`（`modules/registry.py · ModuleRegistry.describe`）。`PUT /config` 对 `disabled_modules` 逐项检查：未知 id → 400；`slot ∈ {runtime.engine, security.pep}` → 400（"通过选择引擎 / 权限模式切换"）；`switchable=false` → 400（`api/admin.py · dsh_set_config`）。

【已实现·注意】注册表层本身**不**做上述检查：`modules/registry.py · discover_and_install` 对 `ctx["disabled"]` 中的任何 id 一律 `set_enabled(False)`。因此通过环境变量 `STAFFDECK_DSH_DISABLED_MODULES` / `Settings.dsh_disabled_modules` 或手工编辑 `staffdeck-runtime.json` 可以停用任何模块（含 K），后果自负（可能 `seal()` 失败或运行期 `AttributeError`）。管理 API 是唯一受保护的入口。
...
|---|---|---|---|
| `runtime.engine` | `engine.legacy`(T) / `engine.dsh`(T) | `EngineHost.open` 取第一个启用者的 `open()` | 不可新增；二选一 |
| `runtime.kernel` | `runtime.coordinator`(K) / `dsh.core`(K) | 信息性 | 不可挂载 |
...
1. 内置 `staffdeck_dsh.modules.builtin.register`（source=`builtin`）；
2. Python entry point 组 **`staffdeck_dsh.modules`**（`ENTRY_POINT_GROUP`；source=`entry_point:<name>`）；加载异常记日志后**重新抛出**，整次装配失败；
3. 规格串 `pkg.module:register`：来自 `Settings.dsh_modules` / 环境变量 `STAFFDECK_DSH_MODULES`（逗号分隔）+ `extra_specs`；规格串格式 `SPEC_RE = ^[A-Za-z_][\w]*(\.[A-Za-z_]\w*)*(:[A-Za-z_]\w*)?$`，`:attr` 省略时默认 `register`（`_load_callable`）；source=规格串原文。管理后台 `extra_modules` 通过 `modules/config.py · apply_overrides` 投影到 `Settings.dsh_modules`，所以走的也是第 3 条。
4. 最后对 `ctx["disabled"]` 中已安装的 id 调用 `set_enabled(False)`。

`pyproject.toml` 写法（第三方包）：

```toml
[project.entry-points."staffdeck_dsh.modules"]
...
- 用 `staffdeck_dsh.modules.registry.manifest(module_id, name, *, kind, slots, summary="", provides=(), requires=(), hooks=(), policy_actions=(), version="1.0.0", contract_version="v1", metadata=None)` 构造清单，或直接构造 `ModuleManifest`。
- 幂等：同一进程多次 restart 会重新调用 `register`（`runtime/assembly.py · restart_dsh_runtime` 每次 `reset_registry()` 后重建）；MUST NOT 依赖模块级全局状态或"只注册一次"假设。
- 快速返回：`register` 只构造对象。
...
- 依赖：A 模块 MUST 只依赖 `staffdeck_dsh.contracts.*`、`staffdeck_dsh.modules.registry.manifest`、`staffdeck_dsh.composition.projection`（授权投影，唯一允许触库的辅助层）以及 `invoke(host, inv)` 时宿主传入的句柄；MUST NOT 直接 import `app.api.*` / `app.core.*` / 各业务 service。
...
2. `host.db` 是宿主为本 TaskFrame 单独开的 `Session`（`bridge/task_agent.py · DshTaskAgent.run` 的 `host_db = Session(bind)`）：provider MAY 在调用期间使用，MUST NOT 在返回后保留引用，MUST NOT 跨线程共享，MUST NOT `commit()`（ledger 负责事务）。
...
5. 禁止：`os.fork`、修改 `sys.path`、修改环境变量、写 `dsh_home`、直接调用 `app.api.*`、捕获并吞掉 `PermissionDenied` / `AuthorizationUnavailable` / `ActivationFenced`（这三类必须向上抛，宿主据此记账）。
6. 日志：`logging.getLogger("staffdeck_dsh.plugins.<module_id>")`；MUST NOT 打印 `SecurityContext.token_id / workload`、模型 API key、工作负载 token。
...
客户端执行：`initialize`（`clientInfo = {name:"staffdeck", version:<staffdeck_dsh 版本>}`）→ `tools/list` → `tools/call capability.describe.v1`。
...
- 端点 `POST /api/enterprise/dsh/hooks/{module_id}/inbound`。
...
- `POST /base/test`（`api/admin.py · dsh_base_test`）：可带未保存的 `base` 补丁；只有当测试值 == 已保存值时才把 `last_test_ok / last_test_at` 写回装配（防止"测通未保存的值"解锁重启）。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `invocation_id` | str | 宿主生成；DSH 路径 = `ctx.trace_id` = `dshcall_<n>_<hex8>`（`CapabilityMcpServer._call_tool`），缺省 `dshcall_<ms>` |
...
- 人工结算：`POST /api/enterprise/dsh/ledger/{id}/reconcile {tenant_id, status: completed|failed, result?}`（`InvocationLedger.reconcile`；非 `outcome_unknown` 行 → 409）；记事件 `invocation_reconciled`。
...
- `ActivationSlot{snapshot, generation, turn_id, session_id, active_sop_id, active_node_id, deadline_monotonic, closed, finish, allowed_next_steps}`；`session_id` 由 `DshTaskAgent.run` 填入，供观察者扇出定位会话（§6.2）。`grants()` / `allowed()` 按当前 SOP 位置取快照授权；`remaining_seconds()` 基于单调时钟。
- `LifecycleFence.check(slot)`：`closed` → "turn is closed"；generation 不匹配 → "stale generation"；`is_cancelled()`（`DshTaskAgent` 提供线程安全的 id 版取消检查）→ "turn cancelled"；截止已过 → "step deadline expired"；均抛 `ActivationFenced`。
...
- `CancelCommand{session_id, turn_id?, reason, requested_by?}` 是数据形状；当前由 legacy `is_chat_turn_cancelled` 驱动（`modules/kernel.py · CancellationModule`；`bridge/task_agent.py · DshTaskAgent.run` 的 `cancelled_threadsafe`）。
- DSH 0.1.2 协议无 cancel 方法：Turn 取消 = 抛 `HarnessExecutionCancelled` 并关闭子进程（`DshTaskAgent.run` finally）。
...
| `dsh_task_finished` | `CapabilityHost._finish_task` | trace + fanout | `{status, next_step_id}` |
...
| `composition_snapshot_compiled` | `DshEngine._ensure_turn_context` | `events.record`（DB） | `{snapshot_id, staff_id, grants, sops, security_profile, execution_engine}` |
| `dsh_process_started` / `dsh_turn_steered` / `dsh_turn_failed` | `DshTaskAgent.run` | trace | `{model, base_url, workspace, boot_ms}` / `{reason, message}` / `{error}` |
| `dsh_turn_started`, `dsh_step_started`, `llm_call_started`, `stream_delta`, `harness_action_created`, `harness_tool_result`, `dsh_assistant_message`, `dsh_turn_ended`, `dsh_hook_event`, `dsh_compaction` | `events/relay.py · relay_event`（DSH `session.event` 翻译） | trace + fanout（`SessionEventRelay.__call__`） | 见 `relay_event` |
...
所有 `CapabilityHost._emit` 事件自动附加 `snapshot_id` 与 `execution_engine:"dsh"`。在 MCP 工作线程发出的 trace 事件先缓冲，附加 `occurred_at`（ISO-8601 UTC），由引擎线程回放持久化（`DshTaskAgent._threadsafe_trace / _flush_trace`）；执行日志按 `occurred_at` 排序（`api/admin.py · _event_ts`）。
...
路径：`Settings.dsh_runtime_config_path` → 环境变量 `STAFFDECK_DSH_RUNTIME_CONFIG` → `<dsh_home 或 cwd>/staffdeck-runtime.json`（`config_path`）。原子写：`<path>.tmp` + `os.replace`（`save_overrides`）。磁盘形状（`RuntimeOverrides.to_stored`）：
...
- 投影：`settings_updates` 把装配映射到 `Settings.dsh_enabled / security_profile / dsh_disabled_modules / dsh_modules / base_*`；`apply_overrides` 直接改写缓存的 `Settings`（真实装配），`settings_view` 返回 `model_copy` 视图（预检 / dry-run，不触碰缓存）。
...
### 7.2 保存 `PUT /api/enterprise/dsh/config`（`api/admin.py · dsh_set_config`）【已实现】
...
### 7.3 Dry-run `POST /api/enterprise/dsh/modules/inspect`（`modules/inspect.py · inspect_spec`）【已实现】
...
### 7.4 归类 `PUT /api/enterprise/dsh/modules/{id}/placement`（`api/admin.py · dsh_set_placement`）【已实现】
...
### 7.5 预检与重启 `POST /api/enterprise/dsh/restart`（`runtime/assembly.py · restart_dsh_runtime`）【已实现】
...
4. `reset_runtime()`（关 MCP 服务端与 DSH 进程）→ `_activate`：`apply_overrides(settings)` → `install_registry(reg)` → `install_profile(profile)`（原子替换，宿主看到的要么是旧装配要么是新装配）→ 若 `dsh_enabled`，`get_runtime`（`EngineUnavailable` 时若 `dsh_fallback_to_legacy` 则只记 `runtime_error`，否则抛出）；
5. 成功：`_applied = wanted`，`restart_count += 1`，`_last_restart_error = None`，返回 `{security_profile, modules, registry_generation, mcp_url?, dsh_root?, dsh_home?, runtime_error?, restarted_at, restart_count, state}`，记事件 `runtime_restarted`；
...
- `GET /status` 报告 `config_pending`、`restarting`、`last_restart_failed`、`registry_generation`、`restart_count`、`base_configured`、`base_last_test_ok` 等；`last_restart_error` / `dsh_root` / `dsh_home` / `mcp_url` 只对管理员返回。Harness v2 为引擎时 `runtime_ok=true`（进程内引擎没有"挂掉"的概念）。
...
| `ENGINE_UNAVAILABLE` | `EngineUnavailable` | DSH 未构建 / 未起（`DshWorkerConfig.validate`；`get_runtime`） | 503 | — |
...
| `PRE_STEP_DENIED` / `DSH_ENGINE_ERROR` | `DshTaskAgent._failed`（TaskExecutionResult.error，非 ModuleResult） | — |
...
# pyproject: [project.entry-points."staffdeck_dsh.modules"] acme = "acme_staffdeck.plugin:register"
...
from staffdeck_dsh.contracts.hooks import HookContext, HookDecision
from staffdeck_dsh.contracts.invocation import ModuleInvocation, ModuleResult
from staffdeck_dsh.contracts.manifest import HookContribution, ModuleKind, SlotName
...
POST /mcp HTTP/1.1
… 同上头 …
X-StaffDeck-Invocation-Id: dshcall_3_9af1c2d4
...
{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{
  ...
  "arguments":{
    ...
    "context":{"tenant_id":"tenant_demo","agent_id":"agent_01","user_id":"user_07","session_id":"sess_9","turn_id":"msg_42","channel":"web",
               "task_frame_id":"tf_1","step_id":"n2","run_id":"run_5","snapshot_id":"a1b2…","trace_id":"dshcall_3_9af1c2d4","deadline_at":"2026-09-03T08:15:00Z","attempt":1},
    "invocation":{"invocation_id":"dshcall_3_9af1c2d4","operation":"knowledge.search/v1","binding_id":null,"side_effecting":false,
  ...
  "_meta":{"progressToken":"dshcall_3_9af1c2d4"}}}
```
...
{"spec_version":"v1",
 ...
 "session_id":"sess_9",
 "module_id":"acme.audit_sink",
 "payload":{"operation":"tool.invoke/v1","resource":"tool_ab12","status":"completed","invocation_id":"dshcall_4_0c1d2e3f",
...
| DSH 约定 | 出处 | 本规范采纳方式 |
|---|---|---|
| SDK 线协议 = JSON-RPC 2.0，每行一条（NDJSON）经 stdio；`initialize` → `session/prompt` → `shutdown`；服务端通知 `session.event` / `session.status` | `packages/sdk/protocol/README.md`（Framing and transport / The SDK methods）；`python/sdk/src/deepseek_harness/client.py · HarnessClient` | 引擎槽内部即用此协议驱动 DSH 子进程（`bridge/worker.py · DshProcess`；`bridge/task_agent.py · DshTaskAgent._run_dsh_turn`）。**不对外暴露**为模块接入协议。 |
